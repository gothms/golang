...
func WithDeadline(parent Context, d time.Time) (Context, CancelFunc) {
	...
	if cur, ok := parent.Deadline(); ok && cur.Before(d) { // 如果parent的截止时间更早，直接返回一个cancelCtx即可
	...
	if dur <= 0 { //当前时间已经超过了截止时间，直接cancel
		...
		return c, func() { c.cancel(false, Canceled, nil) } // 如果当前时间已经超过了截止时间，就直接返回一个已经被 cancel 的 timerCtx
	}
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.err == nil {
		c.timer = time.AfterFunc(dur, func() { // 设置一个定时器，到截止时间后取消
			c.cancel(true, DeadlineExceeded, nil)
		})
	}
	return c, func() { c.cancel(true, Canceled, nil) } // 否则就会启动一个定时器，到截止时间取消这个 timerCtx
}
...
type valueCtx struct {
	Context      // 持有 parent 的 Context
	key, val any // 持有一个 key-value 键值对
}
...
// Value 覆盖了 Value 方法，优先从自己的存储中检查这个 key，不存在的话会从 parent 中继续检查
