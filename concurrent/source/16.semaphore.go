...
func (s *Weighted) Acquire(ctx context.Context, n int64) error {
	s.mu.Lock()
	if s.size-s.cur >= n && s.waiters.Len() == 0 { // fast path, 如果有足够的资源，都不考虑ctx.Done的状态，将cur加上n就返回
		s.cur += n
		s.mu.Unlock()
		return nil
	}
	...
	if n > s.size { // 如果是不可能完成的任务，请求的资源数大于能提供的最大的资源数
		...
		s.mu.Unlock()
		<-ctx.Done() // 依赖ctx的状态返回，否则一直等待
		return ctx.Err()
	}
	// 否则就需要把调用者加入到等待队列中
	ready := make(chan struct{}) // 创建了一个ready chan,以便被通知唤醒
	...
	case <-ctx.Done(): // context的Done被关闭
		...
		default: // 通知waiter
			...
			if isFront && s.size > s.cur { // 通知其它的waiters,检查是否有足够的资源
				s.notifyWaiters()
			}
		}
		s.mu.Unlock()
