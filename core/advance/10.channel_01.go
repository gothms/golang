...
Go 语言最有特色的数据类型
	通道（channel）完全可以与 goroutine 并驾齐驱，共同代表 Go 语言独有的并发编程模式和编程哲学
Don’t communicate by sharing memory; share memory by communicating.
不要通过共享内存来通信，而应该通过通信来共享内存。
	Go 语言的主要创造者之一的 Rob Pike 的至理名言，这也充分体现了 Go 语言最重要的编程理念
	而通道类型恰恰是后半句话的完美实现，我们可以利用通道在多个 goroutine 之间传递数据
唯一自带并发类型
	通道类型的值本身就是并发安全的，这也是 Go 语言自带的、唯一一个可以满足并发安全性的类型

通道容量
	指通道最多可以缓存多少个元素值
	虽然这个参数是int类型的，但是它是不能小于0的
...
问题解析
	...
	特性二
		...
		既是为了保证通道中元素值的完整性，也是为了保证通道操作的唯一性
...
问题 1：发送操作和接收操作在什么时候可能被长时间的阻塞？
	...
	vs
		缓冲通道
			异步的方式传递数据
			在大多数情况下，缓冲通道会作为收发双方的中间件，元素值会先从发送方复制到缓冲通道，之后再由缓冲通道复制给接收方
	...
	通道类型是引用类型，所以它的零值就是nil
		当我们只声明该类型的变量但没有用make函数对它进行初始化时，该变量的值就会是nil
问题 2：发送操作和接收操作在什么时候会引发 panic？
	...
	关闭通道
		...
		当我们把接收表达式的结果同时赋给两个变量时，第二个变量的类型就是一定bool类型
...
思考
	...
	2.元素值在经过通道传递时会被复制，那么这个复制是浅表复制还是深层复制呢？
		...
		指针
			创建一个指针的内存开销绝大多数情况下要比重新开辟一块内存再复制数据性能高
			传指针要自己保证安全，如原始数据放篡改
